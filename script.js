...
const slideSection = (line) => {
  ...
  if ((Math.floor(line / sizeArticle) >= 0) && (Math.floor(line / sizeArticle) <= articles.length - 1)) {
  ...
  if ((Math.floor(line / sizePictures) >= 0) && (Math.floor(line / sizePictures) <= articlesImg.length - 1)) {
...
//tablica z nazwami
const nameBeers = ["", "Porter Bałtycki", "potrójny złoty", "poczwórny bursztynowy", "belgian ipa", "porter malinowy", "porter płatki dębowe", "russian imperial stout", "barley wine", "Sherry Oloroso"];
